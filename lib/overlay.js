...
function installPrefsOverlay(win) {
  ...
  if (xulRuntime.OS === "WINNT") {
    pi = loadStylesheet(win, "skin/winstripe/preferences.css");
  } else
  if (xulRuntime.OS === "Darwin") {
    pi = loadStylesheet(win, "skin/pinstripe/preferences.css");
  } else  {
    pi = loadStylesheet(win, "skin/gnomestripe/preferences.css");
  }
  ...
  pane.setAttribute('src', require("self").data.url("content/preferences.xul"));
...
function installOverlay(win) {
  ...
  **/
  ...
  try {
    ...
    Services.prefs.setCharPref("services.share.shareURL", require("self").data.url("ui/share/index.html"));
  }
  //try {
...
exports.installPrefsOverlay = installPrefsOverlay;
