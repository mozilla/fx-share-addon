...
SharePanel.prototype = {
  ...
  _messageListener: function(event) {
    ...
    var topic = message.cmd;
    if (topic && PANEL_MESSAGE_TOPICS.indexOf(topic) >= 0) {
      try {
        // XXX f1 uses topic
        this[topic](message.data);
      } catch (ex) {
        dump("ERROR: "+ex+"\n");
        console.error("Handler of SharePanel command", topic, "failed:", ex, ex.stack);
      }
    } else {
  ...
  result: function (data) {
    ...
    this.updateStatus({statusCode: SHARE_DONE});
   ...
   */
  updateStatus: function (statusRecord) {
