...
***/
  ...
  /* OWA interface to our panel object */
  get anchor() { return this.button },
  updateargs: function(contentargs) {
    let optBuilder = new PageOptionsBuilder(this.gBrowser);
  ...
  onresult: function(req) {
    return this.onMediatorCallback(req);
  },
  /* END OWA INTERFACE */
  ...
  onMediatorCallback: function(message) {
      //dump("onMediatorCallback got data "+JSON.stringify(message)+"\n");
    // Bail if this isn't a valid message topic.
    if (!message.cmd||
