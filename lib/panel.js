...
***/
  ...
  updateStatus: function (statusData, success) {
    let shareUrl = statusData && statusData.length > 3 ? statusData[3] : null,
    ...
    if (typeof(statusData) === 'undefined') {
      statusData = shareState ? shareState.status : [SHARE_DONE,,,shareUrl];
    }
    status = statusData[0];
    ...
    if (contentTab) {
      contentBrowser = this.gBrowser.getBrowserForTab(contentTab);
      nBox = this.gBrowser.getNotificationBox(contentBrowser);
      notification = nBox.getNotificationWithValue("mozilla-f1-share-error");

      // we only deal with UI if we have a contentTab that matches the shareUrl
