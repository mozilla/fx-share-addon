...
SharePanel.prototype = {
  ...
  onGenerateBase64Preview: function(imgUrl) {
    ...
    img.onload = function() {
      ...
      self.panel.port.emit("base64Preview", dataUrl);
    };
    img.src = imgUrl;
  },
...
PageOptionsBuilder.prototype = {
  ...
  getCanonicalURL: function() {
    ...
    let pageLocation = this.gBrowser.contentWindow.location;

    for (i = 0; i < links.length; i++) {
      let link = links[i].getAttribute("content");
      if (link.indexOf("/") == 0) {
        link = pageLocation.protocol + "//" + pageLocation.host + link;
      }
      if ((content = this._validURL(link))) {
        return content;
      }
    }
