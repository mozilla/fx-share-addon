...
SharePanel.prototype = {
  ...
  _messageListener: function(event) {
    if (event.origin != this.origin && event.origin != "resource://openwebapps/service") {
        dump("SharePanel: cannot accept postMessage from "+event.origin+" to "+this.origin+"\n");
        return;
    }
    var message = JSON.parse(event.data);
    let topic = message.topic || message.cmd
