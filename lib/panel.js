...
SharePanel.prototype = {
  ...
  inject: function() {
    // pretty much just injecting oauth support so the owa iframes can use it
    let self = this;
    this.window.appinjector.register({
        apibase: "navigator.apps.oauth", name: "authorize", script: null,
        getapi: function (contentWindowRef) {
            return function (config, callback) {
                self.oauthAuthorize(config, callback);
            }
        }
    });    
    this.window.appinjector.register({
        apibase: "navigator.apps.oauth", name: "call", script: null,
        getapi: function (contentWindowRef) {
  ...
  on_storeGet: function storeGet(key) {
    //SecureKeyValueStore.get(key, (function(value) {
      this.sendMessage("storeGetReturn", {key: key, value: null});
    //}).bind(this));
  },

  on_storeSet: function storeSet(data) {
    //SecureKeyValueStore.set(data.key, data.value);
  },

  on_storeRemove: function storeRemove(key) {
    //SecureKeyValueStore.remove(key);
  },

  on_storeRemoveAll: function storeRemoveAll() {
  ...
  on_result: function (data) {
    ...
    this.on_updateStatus({statusCode: SHARE_DONE});
    ...
    // close the panel.
    MediatorPanel.prototype.on_result.apply(this, arguments);
  },
   ...
   */
  on_updateStatus: function (statusRecord) {
    let statusCode = statusRecord.statusCode,
   ...
   */
  on_generateBase64Preview: function (imgUrl) {
    let self = this;
    let img = new this.window.Image();
    img.onload = function () {

      let canvas = self.window.document.createElementNS(NS_XUL, "canvas"),
          w = img.width,
          h = img.height,
  ...
  _makeOauthProvider: function(config) {
    try {
      ...
      // oauthorizer.
      var svc = OAuthConsumer.makeProvider("f1-"+config.name, config.displayName, config.key, config.secret, config.completionURI, config.calls)
      svc.version = config.version;
      svc.tokenRx = new RegExp(config.tokenRx, "gi");

      if (config.params)
          svc.requestParams = config.params;
      return svc;
    } catch(e) {
      dump(e+"\n");
    }
  ...
  oauthAuthorize: function(config, callback) {
    try {
      dump("oauthAuthorize called with message "+JSON.stringify(config)+"\n");
      var svc = this._makeOauthProvider(config.oauth);
      var self = this;
  ...
  on_installApp: function(message) {
