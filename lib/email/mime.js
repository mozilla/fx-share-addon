...
// Does the passed value require utf-8 encoding?
// If so, return it.  If not, return null (so the
// original value can be used)
function maybeMakeUtf8(val) {
  // hrmph - previously we tried using encode_utf8 and checking if the result
  // string was identical to the input string, but this failed for chars like
  // '@' etc.  So for now we explicitly check each char in the string is 7bit.
  // A "real" utf-8 encoder could probably do better.
  val = val.toString();
  ...
  // all chars are 7bit - no need to use utf-8 for it.
  return null;
}

function encodeHeaderValue(val) {
  let utf8 = maybeMakeUtf8(val);
  if (utf8===null) {
    // 7 bit - value is ok as-is
    return val;
  }
  // gotta do the encoded-word magic
  return "=?utf-8?B?" + base64Encode(utf8) + "?=";
}
...
Message.prototype = {
  ...
  */
  addHeader: function(name, value, params) {
    ...
    if (value != null) {
      parts.unshift(encodeHeaderValue(value));
    }
...
function MimeText(text, subtype) {
  ...
  let utf8_payload = maybeMakeUtf8(text);
