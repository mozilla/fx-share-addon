...
SslSmtpClient.prototype.connect = function(smtpArgs, cb, cberr, logging) {
    ...
    try {
        ...
        cberr({type: 'runtime_error', message: err});
        return;
    }
    this.commands = []
    this.response_data = "";
...
SslSmtpClient.prototype.authenticate = function(args, cb, cberr) {
    if ('basic' in args) {
        var u = encode_utf8(args.basic.username);
        var p = encode_utf8(args.basic.password);
        var auth = btoa("\0" + u + "\0" + p);
        this.sendCommand("AUTH PLAIN " + auth, this.onLogin, [cb, cberr]);
    } else {
