...
const {parseaddr} = require("./addressutils");
...
SslSmtpClient.prototype.sendMessage = function(to, payload, on_success, on_error) {
    ...
    var client = this;
    var fromAddress = parseaddr(this.smtpArgs.email)[1];
    if (!fromAddress) {
        on_error({type: 'value_error', message: "invalid 'from' address: " + this.smtpArgs.email});
        return;
    }
    client.sendCommand("MAIL FROM: <" + encode_utf8(fromAddress) + "> BODY=8BITMIME", function(reply) {
        var code = reply[0].split(" ", 1);
        ...
        if("to" in send_cmd && send_cmd["to"].length > 0) {
            ...
            let toString = send_cmd.to.pop();
            let toAddress = parseaddr(toString)[1];
            if (!toAddress) {
                send_cmd['error']({type: 'value_error', message: "A recipient address is invalid: " + toString});
                return;
            }
            client.sendCommand("RCPT TO: <" + encode_utf8(toAddress) + ">", arguments.callee, [], true);
