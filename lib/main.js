...
*/
  ...
  init: function() {
    ...
    // inject the email send api
    // XXX TEMPORARY HACK to get email working
    ...
    this.window.appinjector.register({
      apibase: "navigator.mozApps.services.sendEmail",
      name: "call",
      script: null,
      getapi: function(contentWindowRef) {
        return function(svc, message, callback) {
          ...
          let on_disconnect = function() {
            if (!finished) {
              callback({"error": "premature disconnection"});
            }
          }
          ...
          let on_connected = function() {
            client.authenticate(svc,
              function() {
                // now we can send the message - put together the mime bits.
                ...
                if (message.thumbnail) {
                  ...
                  let imagepart = new MimeEncoded("image", "png", message.thumbnail, "base64")
          ...
          client.connect(svc, on_connected, callback, true);
        } // end api
      } // getapi
    });
