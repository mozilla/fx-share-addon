...
const MODE_FLAGS = FileUtils.MODE_WRONLY | FileUtils.MODE_CREATE | FileUtils.MODE_TRUNCATE;
...
function _notifyKeyChanged(subject) {
  ...
  subject.wrappedJSObject = subject; // XXX is this safe?
...
const SecureFileStore = {
  ...
  fetch: function fetch(aCallback) {
    ...
    function onFetch(aInputStream, aStatusCode, aRequest) {
      ...
      let ciphertext = NetUtil.readInputStreamToString(aInputStream, aInputStream.available());
  ...
  store: function store(aData, aCallback) {
    ...
    let outputStream = Cc["@mozilla.org/network/safe-file-output-stream;1"].createInstance(Ci.nsIFileOutputStream);
    outputStream.init(this._file, MODE_FLAGS, FILE_PERMS, outputStream.DEFER_OPEN);
...
XPCOMUtils.defineLazyServiceGetter(SecureFileStore, "_crypto", "@mozilla.org/login-manager/crypto/SDR;1", "nsILoginManagerCrypto");
